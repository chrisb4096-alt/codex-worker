...
- **Proof over trust.** Session and usage footers make every leg auditable: which Codex session ran, what it cost, whether it ran at all. Output without proof is rejected.
...
- The runner and hooks never see or store credentials; Codex auth lives wherever `codex login` put it (`~/.codex/`), outside this stack.
